...
(function(){
 ...
 function setMobileView(places){document.body.classList.toggle('map-show-places',places);$('#mobile-map-view').setAttribute('aria-pressed',!places);$('#mobile-places-view').setAttribute('aria-pressed',places);if(!places)setTimeout(()=>map?.resize(),50);}
 $('#mobile-map-view').onclick=()=>setMobileView(false);
 $('#mobile-places-view').onclick=()=>setMobileView(true);
 $('#mobile-map-close').onclick=()=>setExpanded(false);
 ...
 window.focusMapArea=id=>{if(phone()&&expanded&&id!=='all')setMobileView(true);if(!map)return;const r=MEDIA.regions[id];map.flyTo({...r?{center:r.center,zoom:r.zoom}:homeView(),pitch:terrain?(id==='all'?homeView().pitch:id==='austria'?64:48):0,bearing:id==='all'?homeView().bearing:id==='austria'?35:-12,duration:reduced?0:1500,essential:false});};
 ...
 function setExpanded(value){expanded=value;setMobileView(false);document.body.classList.toggle('map-immersive',value);$('#map-expand').textContent=value?'✕ Close map':'⛶ Expand';$('#map-expand').setAttribute('aria-pressed',value);setTimeout(()=>map?.resize(),50);}
